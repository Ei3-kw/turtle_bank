...
const GoalSaverPage = () => {
    ...
    const handleAddBehavior = (e) => {
        ...
        if (newCategory && newPercentage && newAmount) {
            setSpendingBehaviors({
                ...spendingBehaviors,
                [newCategory]: {
                    Percentage: parseFloat(newPercentage) / 100,
    ...
    const handleRemoveBehavior = (category) => {
        const updatedBehaviors = {...spendingBehaviors};
        delete updatedBehaviors[category];
        setSpendingBehaviors(updatedBehaviors);
    };

    const handleEditBehavior = (category, field, value) => {
        setSpendingBehaviors({
            ...spendingBehaviors,
            [category]: {
                ...spendingBehaviors[category],
                [field]: field === 'Percentage' ? parseFloat(value) / 100 : parseFloat(value)
            }
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        const formattedData = {
            ...
            spendingBehavior: spendingBehaviors,
    ...
    const renderComparisonChart = () => {
        if (!aiSuggestion || !aiSuggestion.SpendingCategory) return null;
                ...
                <table className="min-w-full bg-white">
                    ...
                    {Object.entries(aiSuggestion.SpendingCategory).map(([category, values]) => (
                        <tr key={category}>
                            ...
                            <td className="px-4 py-2 border">{(values.Percentage * 100).toFixed(1)}%</td>
                        </tr>
                    ))}
                    </tbody>
                </table>
                <div className="mt-4">
                    <p className="font-bold">Goal Amount: ${aiSuggestion.GoalAmount?.toFixed(2) || 'N/A'}</p>
                    <p className="font-bold">Proposed Monthly Saving: ${aiSuggestion.ProposedMonthlySaving?.toFixed(2) || 'N/A'}</p>
                    <p className="font-bold">Proposed Monthly Expense: ${aiSuggestion.ProposedMonthlyExpense?.toFixed(2) || 'N/A'}</p>
                    <p className="font-bold mt-2">Overall Suggestion:</p>
                    <p>{aiSuggestion.OverallSuggestion || 'No suggestion available'}</p>
                </div>
                {aiSuggestion.Products && aiSuggestion.Products.length > 0 && (
                    <div className="mt-4">
                        <h3 className="text-xl font-bold mb-2">Suggested Products</h3>
                        <ul className="list-disc pl-5">
                            {aiSuggestion.Products.map((product, index) => (
        ...
        <div className="max-w-4xl mx-auto mt-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                    ...
                    {Object.entries(spendingBehaviors).map(([category, behavior]) => (
                        <div key={category} className="flex items-center space-x-2 mb-2">
                            ...
                            <input
                                type="number"
                                value={(behavior.Percentage * 100).toFixed(1)}
                                onChange={(e) => handleEditBehavior(category, 'Percentage', e.target.value)}
                                className="w-20 px-2 py-1 border rounded"
                                step="0.1"
                            />
                            <input
                                type="number"
                                value={behavior.Amount.toFixed(2)}
                                onChange={(e) => handleEditBehavior(category, 'Amount', e.target.value)}
                                className="w-24 px-2 py-1 border rounded"
                                step="0.01"
                            />
                            <button
                                type="button"
                                onClick={() => handleRemoveBehavior(category)}
            ...
            {aiSuggestion && aiSuggestion.SpendingCategory && renderComparisonChart()}
            {aiSuggestion && aiSuggestion.SpendingCategory && renderSuggestionTable()}
        </div>
    );
};
